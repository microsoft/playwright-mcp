...
test.describe('EnhancedErrorHandler', () => {
  test('should enhance Playwright errors with diagnostic information', async ({ page }) => {
    ...
    const originalError = new Error('Element not found: button[data-test="missing"]');
    ...
    expect(enhancedError.suggestions).toContain('Try using one of the 1 alternative elements found');
  });

  test('should provide timeout-specific enhancements', async ({ page }) => {
    await page.goto('data:text/html,<div><iframe src="data:text/html,<button>In Frame</button>"></iframe></div>');
    ...
    expect(enhancedError.suggestions).toContain('Element might be inside an iframe');
  });

  test('should detect when elements are in different context', async ({ page }) => {
    await page.goto('data:text/html,<div><iframe id="frame" src="data:text/html,<button id=inner>Inside</button>"></iframe><button id="outer">Outside</button></div>');
    ...
    expect(enhancedError.suggestions).toContain('element might be in a different frame - use frameLocator()');
  });

  test('should provide performance insights for slow operations', async ({ page }) => {
    await page.goto('data:text/html,<div><button onclick="setTimeout(() => alert(\'slow\'), 2000)">Slow Action</button></div>');
    ...
    expect(enhancedError.suggestions.some(s => s.includes('Operation took longer than expected'))).toBe(true);
  });

  test('should integrate with existing tool error handling', async ({ page }) => {
    await page.goto('data:text/html,<div><button disabled>Disabled</button></div>');
    ...
    expect(enhancedError.suggestions).toContain('Element appears to be disabled');
  });
});
