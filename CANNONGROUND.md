...
## Auth-bridged sites (persistent profile)

The Playwright MCP's per-workspace persistent profile (`%LOCALAPPDATA%\ms-playwright\mcp-chrome-<workspace-hash>\`) carries authenticated session cookies across `browser_close` + MCP session restart, as long as the cwd's workspace-hash is stable. This workspace's hash is `a7154fb` (clone at `~/CannonGround/`). Sites authenticated through this profile so far:

| Site | Login URL | Probe URL | Verified | Notes |
|---|---|---|---|---|
| Bravos Research | (subscription page) | `https://bravosresearch.com/research/` | PBPM-M0 2026-05-24 | One-time manual login through `browser_navigate` then ATI table renders authenticated. |
| Koyfin | `https://app.koyfin.com/login` | `https://app.koyfin.com/myw/b029ca33-415c-4bf6-b56b-e45ceaa0c14e` | KPW-M0 2026-05-25 | One-time interactive login; persistence verified across page-close + re-navigate. Auth-gated nav signature (My Koyfin / My Portfolio / My Screens / My Dashboards / Advisor Tools) used as cheap heartbeat probe. Detail probe + gate script: `koyfin-watchlist/scripts/koyfin_auth_probe.py`. |

**Session-timeout posture (TBD)** — Koyfin session-expiration cadence unknown at M0 ship. Calendar marker: re-validate at KPW-M4 (retirement gate, after ≥5 parallel-run fires). Worst-case the heartbeat probe hard-aborts the routine, Cannon re-logs interactively, next fire resumes — same shape as Enphase refresh-token rotation (~30 days, [`project_enphase_connector`](../.claude/projects/C--Users-Canno-CannonGround/memory/project_enphase_connector.md)).
...
- **2026-05-25 — KPW-M0 (downstream)**: koyfin-playwright-migration auth-bridge ratified the same Candidate A persistent-profile pattern PBPM-M0 used for Bravos. Triggered by 2026-05-25 first `market-snapshot` (v1.0) fire double-aborting — Claude-in-Chrome path stalled at 33/350 NDV rows under intensive background throttle + view-switch dropdown items had zero bounding rects; Playwright path loaded clean but landed on the logged-out "My Watchlists is only for registered Koyfin users" gate. One-time interactive login at `https://app.koyfin.com/login` (workspace-hash `a7154fb`); cookie banner dismissed via privacy-preserving `.cky-btn-reject`. Verification probe consumes a `browser_evaluate(filename=.playwright-mcp/koyfin-auth-probe.json)` extract (5 auth-nav markers: My Koyfin / My Portfolio / My Screens / My Dashboards / Advisor Tools) + paywall-text + login-button checks; persistence verified across `browser_close` + re-navigate (5/5 auth nav hits both pre- and post-close). Routine-side gate script: `koyfin-watchlist/scripts/koyfin_auth_probe.py` (atomic-or-fail, exit 0=ok / exit 1=abort; threshold ≥3/5 auth-nav markers to tolerate Koyfin nav A/B drift). One config-patch attempted-and-reverted during M0: `--headed` flag added to MCP args, but `@playwright/mcp@0.0.75` defaults to headed (the opt-in flag is `--headless`) and `--headed` is an invalid arg that crashes the MCP at startup. Reverted to default args. M1 (NDV extractor parser) + M2 (CSV download via Playwright) + M3 (routine docs rewrite for both `koyfin-midday-snapshot.md` v1.10→v1.11 and `market-snapshot.md` v1.0→v1.1) + M4 (retirement) remain queued.
