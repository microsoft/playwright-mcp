...
export class UnifiedDiagnosticSystem {
    ...
    async initializeComponents() {
        ...
        try {
            ...
            this.initializationError = error instanceof DiagnosticError
                ? error
                : DiagnosticError.from(error, 'UnifiedSystem', 'initializeComponents', { timestamp: Date.now() });
            throw this.initializationError;
        }
        finally {
    ...
    async performInitialization() {
        ...
        const partiallyInitializedComponents = [];
        try {
            ...
            const diagnosticError = new DiagnosticError(`Component initialization failed: ${error instanceof Error ? error.message : String(error)}`, {
                timestamp: Date.now(),
    ...
    collectDisposableComponents(components) {
        // Add currently initialized components that have dispose methods
        if (this.resourceManager) {
            components.push(this.resourceManager);
        }
        if (this.pageAnalyzer) {
            components.push(this.pageAnalyzer);
        }
        if (this.parallelAnalyzer) {
            components.push(this.parallelAnalyzer);
        }
        if (this.resourceMonitor) {
            components.push(this.resourceMonitor);
        }
    }
    ...
    async ensureInitialized() {
        ...
        if (!this.isInitialized) {
            throw new DiagnosticError('UnifiedSystem components are not initialized', {
                timestamp: Date.now(),
                component: 'UnifiedSystem',
    ...
    async executeOperation(operation, component, fn, options) {
        ...
        const componentConfig = this.configManager.getComponentConfig(component.toLowerCase());
        ...
        try {
            ...
            const diagnosticError = error instanceof DiagnosticError
                ? error
                : DiagnosticError.from(error, component, operation, { executionTime, timestamp: startTime });
    ...
    recordOperation(operation, component, executionTime, success) {
        ...
        if (this.configManager.getConfig().runtime.enableAdaptiveThresholds) {
            const recentOps = this.operationHistory.filter(op => op.operation === operation &&
    ...
    async analyzePageStructure(forceParallel) {
        ...
        if (shouldUseParallel) {
            ...
            return this.executeOperation('analyzePageStructure', 'PageAnalyzer', () => this.pageAnalyzer.analyzePageStructure());
        }
    }
    async findAlternativeElements(searchCriteria) {
        await this.ensureInitialized();
        return this.executeOperation('findAlternativeElements', 'ElementDiscovery', () => this.elementDiscovery.findAlternativeElements(searchCriteria));
    }
    async analyzePerformanceMetrics() {
        await this.ensureInitialized();
        return this.executeOperation('analyzePerformanceMetrics', 'PageAnalyzer', () => this.pageAnalyzer.analyzePerformanceMetrics());
    }
    // Resource management with automatic cleanup
    async createSmartHandle(creator, disposer, options) {
        await this.ensureInitialized();
    ...
    getConfigurationReport() {
        ...
        let configurationStatus = 'default';
        ...
        Object.keys(expectedExecutionTimes).forEach(key => {
            const expected = expectedExecutionTimes[key];
            const actual = actualAverages[key];
