...
export const browserDiagnose = defineTabTool({
    ...
    handle: async (tab, params, response) => {
        const { searchForElements, includePerformanceMetrics, includeAccessibilityInfo, includeTroubleshootingSuggestions, diagnosticLevel = 'standard', useParallelAnalysis = false, useUnifiedSystem = true, configOverrides, includeSystemStats = false } = params;
        try {
            ...
            try {
                ...
                let resourceUsageInfo;
                let systemHealthInfo;
                if (unifiedSystem) {
                    // Use unified system for enhanced analysis with error handling
                    reportSections.push('# Unified Diagnostic System Report', `**Unified System Status:** Active with enhanced error handling and monitoring`, `**Configuration:** ${configOverrides ? 'Custom overrides applied' : 'Default settings'}`, '');
                    // Add configuration override details to the report
                    if (appliedOverrides.length > 0) {
                        reportSections.push('## Applied Configuration Overrides');
                        appliedOverrides.forEach((override) => {
                    ...
                    if (includeSystemStats) {
                        ...
                        if (systemHealthInfo.issues.length > 0) {
                            ...
                            systemHealthInfo.issues.forEach((issue) => {
                        ...
                        if (systemHealthInfo.recommendations.length > 0) {
                            ...
                            systemHealthInfo.recommendations.forEach((rec) => {
                    ...
                    if (parallelRecommendation.recommended || useParallelAnalysis) {
                        ...
                        reportSections.push('# Enhanced Diagnostic Report (Parallel Analysis)', `**Parallel Analysis Execution Time:** ${parallelResult.executionTime}ms`, `**Resource Monitoring:** ${resourceUsageInfo ? 'Enabled' : 'Disabled'}`, '');
                        ...
                        reportSections.push('# Standard Diagnostic Report', `**Parallel Analysis:** Not recommended - ${parallelRecommendation.reason}`, '');
                    }
                }
                else {
                ...
                if (diagnosticLevel === 'basic') {
                    reportSections.push('# Basic Diagnostic Report', `**URL:** ${tab.page.url()}`, '', '## Critical Information');
                ...
                // Standard level and above
                    ...
                    reportSections.push('# Page Diagnostic Report', `**URL:** ${tab.page.url()}`, `**Title:** ${await tab.page.title()}`, '', '## Page Structure Analysis', `- **IFrames:** ${diagnosticInfo.iframes.count} iframes detected: ${diagnosticInfo.iframes.detected}`, `- **Accessible iframes:** ${diagnosticInfo.iframes.accessible.length}`, `- **Inaccessible iframes:** ${diagnosticInfo.iframes.inaccessible.length}`, '', `- **Total visible elements:** ${diagnosticInfo.elements.totalVisible}`, `- **Total interactable elements:** ${diagnosticInfo.elements.totalInteractable}`, `- **Elements missing ARIA:** ${diagnosticInfo.elements.missingAria}`, '');
                }
                ...
                if ((includePerformanceMetrics || diagnosticLevel === 'detailed' || diagnosticLevel === 'full')
                    ...
                    try {
                        ...
                        if (comprehensiveMetrics.domMetrics.largeSubtrees.length > 0) {
                            ...
                            comprehensiveMetrics.domMetrics.largeSubtrees.slice(0, 3).forEach((subtree, index) => {
                        ...
                        if (diagnosticLevel === 'full') {
                            ...
                            if (comprehensiveMetrics.layoutMetrics.fixedElements.length > 0) {
                                ...
                                comprehensiveMetrics.layoutMetrics.fixedElements.slice(0, 5).forEach((element, index) => {
                            ...
                            if (comprehensiveMetrics.layoutMetrics.highZIndexElements.length > 0) {
                                ...
                                comprehensiveMetrics.layoutMetrics.highZIndexElements.slice(0, 5).forEach((element, index) => {
                        ...
                        if (comprehensiveMetrics.warnings.length > 0) {
                            ...
                            comprehensiveMetrics.warnings.forEach((warning) => {
                    ...
                    if (resourceUsageInfo && useParallelAnalysis) {
                        ...
                        if (resourceUsageInfo.analysisSteps.length > 0) {
                            ...
                            resourceUsageInfo.analysisSteps.forEach((step, index) => {
