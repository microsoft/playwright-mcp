    ...
    async finish() {
        ...
        if (shouldIncludeSnapshot && this._context.currentTab()) {
            ...
            if (options?.selector || options?.maxLength) {
                ...
                this._tabSnapshot = await this._context.currentTabOrDie().capturePartialSnapshot(options.selector, options.maxLength);
            }
            else {
        ...
        if (this._expectation.diffOptions?.enabled) {
            try {
                ...
                this._diffResult = await Response.diffDetector.detectDiff(currentContent, this.toolName, diffOptions);
            }
            catch (error) {
...
\`\`\``);
    ...
    buildContentForDiff() {
        ...
        if (this._tabSnapshot?.consoleMessages.length && this._expectation.includeConsole) {
            const filteredMessages = this.filterConsoleMessages(this._tabSnapshot.consoleMessages, this._expectation.consoleOptions);
            if (filteredMessages.length) {
