...
Install to any AI agent host (Claude Code, Cursor, Claude Desktop, Windsurf, VS Code, and [13 more](https://github.com/pea3nut/agent-get)) with a single command via [agent-add](https://github.com/pea3nut/agent-get):

```bash
npx -y agent-add --mcp '{"playwright":{"command":"npx","args":["@playwright/mcp@latest"]}}'
```

`agent-add` auto-detects your AI host and writes the config to the correct location.
