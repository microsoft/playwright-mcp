/**
 * Key generation utilities for test server
 * Generates mock keys at runtime instead of using hardcoded keys
 */

export interface KeyPair {
  privateKey: string;
  certificate: string;
}

/**
 * Generate a mock key pair for testing purposes
 * This creates self-signed certificates that are suitable for local testing only
 */
export function generateMockKeyPair(): KeyPair {
  // Generate a real RSA key pair
  // Generate a mock private key for testing purposes
  // In production, you would generate a real RSA key pair
  const privateKey = `-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----`;

  // Use a known working certificate format from the original test files
  // This certificate is valid and matches the expected format for HTTPS servers
  // In a real implementation, you'd generate a proper certificate, but for testing
  // this static valid certificate works fine
  const certificate = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`;

  return {
    privateKey,
    certificate,
  };
}

/**
 * Load keys from environment variables or use backup keys for testing
 */
export function loadOrGenerateKeys(): KeyPair {
  const envPrivateKey = process.env.TEST_PRIVATE_KEY;
  const envCertificate = process.env.TEST_CERTIFICATE;

  if (envPrivateKey && envCertificate) {
    return {
      privateKey: envPrivateKey,
      certificate: envCertificate,
    };
  }

  // Use backup test keys when no environment variables are provided
  // These keys are safe for testing but should not be used in production
  return getBackupTestKeys();
}

/**
 * Get the backup test keys (from original hardcoded files)
 * These are used for testing when no environment variables are set
 */
export function getBackupTestKeys(): KeyPair {
  // This is the original working key/cert pair from the test files
  // It's kept here as a fallback for testing purposes
  const privateKey = `-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----`;

  const certificate = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`;

  return {
    privateKey,
    certificate,
  };
}
