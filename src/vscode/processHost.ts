...
import { ProtocolResponse } from './process.js';
...
export class ProcessHost extends EventEmitter {
  ...
  constructor(runnerScript: string, env: Record<string, string | undefined>) {
    super();
  ...
  async startRunner(runnerParams: any, options: { onStdOut?: (chunk: Buffer | string) => void, onStdErr?: (chunk: Buffer | string) => void } = {}): Promise<ProcessExitData | undefined> {
    ...
    this.process = child_process.fork(new URL('./process.js', import.meta.url), {
      detached: false,
      env: {
        ...process.env,
        ...this._extraEnv,
